...
└── active/
    ├── <widget-name> -> ../store/<widget-name>/<active-version>/<entrypoint>
    └── shared/
        └── <module>.lua -> .../store/<package>/<active-version>/<export-file>.lua
```

`store/<name>/<version>/` is the exact validated package tree. EasyBar does not build a second source
copy or runtime projection inside the committed version.

For widget packages, `active/<widget-name>` is a relative symbolic link directly to the entrypoint
declared by `package.toml`. Declared exports are relative symbolic links below `active/shared/` that
point directly to their declared files in the committed package version. EasyBar therefore activates
only manifest-declared Lua files instead of recursively exposing a package directory as widget code.

The Lua runtime lists only top-level widget activation symlinks below `active/`; it does not
recursively scan `store/`.
...
The staged directory is the validated package tree itself. Only after it is prepared does EasyBar
rename it to the committed version path.

For a new version, the existing versions remain untouched while the new version is prepared and
committed. For a widget package, EasyBar then replaces `active/<name>` with a newly staged symbolic
link to the committed entrypoint file. Declared exports are switched separately below
`active/shared/` to their committed package files.
