...
│           └── runtime/
└── active/
    ├── <widget-name> -> ../store/<widget-name>/<active-version>/runtime
    └── shared/
        └── <module>.lua -> .../store/<package>/<active-version>/source/<file>.lua
```

`store/<name>/<version>/source/` is the exact validated package tree. EasyBar does not rewrite its
contents after validation.

`store/<name>/<version>/runtime/` is generated by EasyBar. For widget packages it contains the
entrypoint plus non-Lua files copied from `source/`; other Lua files and declared export targets stay
in `source/`. Library packages do not have a directly executed widget projection, so their
`runtime/` directory remains empty.

`active/<widget-name>` is a relative symbolic link to the committed widget version's `runtime/`
directory. Declared package exports are relative symbolic links below `active/shared/` that point to
their declared files in the committed version's `source/` directory. EasyBar loads this activation
tree in addition to the manual `[app].widgets_dir`; it never scans `store/` directly.
...
The staged directory contains both `source/` and `runtime/`. EasyBar first copies the validated
package tree into `source/`, then generates `runtime/` from the manifest. Only after both directories
are prepared does EasyBar rename the staged directory to the committed version path.

For a new version, the existing versions remain untouched while the new version is prepared and
committed. For a widget package, EasyBar then replaces the `active/<name>` symlink with a newly
staged symlink that points to the committed version's `runtime/` directory. Declared exports are
switched separately below `active/shared/` to their files in `source/`.
