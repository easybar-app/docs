...
Built-in widgets and native groups share the same layout vocabulary:

- `margin_x` and `margin_y` control outer spacing around a widget or group;
- `padding_x` and `padding_y` control space inside its visual container;
- `spacing` controls the gap between children inside a group or layout.

For the native `spaces` widget, `[builtins.spaces]` owns the outer placement and box model while
`[builtins.spaces.layout]` controls layout inside each workspace pill.
