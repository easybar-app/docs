...
discovery follows the widget symlinks below `active/`. Each widget symlink targets only that
version's generated `runtime/` directory, so the complete package contents in `source/` are not part
of the discovery tree. Manual widget discovery does not follow package activation symlinks.

The managed activation tree contains widget symlinks to `runtime/` and declared export symlinks to
files below `source/`. The Lua runtime never scans `store/` directly.
...
`source/` is the exact validated package tree. `runtime/` is generated by EasyBar and is the only
part of a widget package exposed to direct widget discovery. For widgets it contains the entrypoint
plus non-Lua files; other Lua files and declared export targets remain in `source/`.

For a widget package, `active/<name>` is a relative symlink to the committed version's `runtime/`
directory. Declared exports are symlinks below `active/shared/` that point to their declared files in
the committed version's `source/` directory. Managed discovery follows widget activation symlinks
and prunes `shared/`, so package source is never executed merely because it was installed.
...
- managed discovery follows widget activation symlinks into `runtime/` and excludes `shared/`
