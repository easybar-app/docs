...
lua_generator="$easybar_root/scripts/generate/lua_docs.py"
if [[ ! -f "$lua_generator" ]]; then
  echo "EasyBar Lua documentation generator not found: $lua_generator" >&2
  exit 1
fi

"$python" "$lua_generator" --output "$lua_output"
if [[ ! -f "$lua_output/index.md" ]]; then
  echo "EasyBar did not generate the Lua reference at $lua_output" >&2
  exit 1
fi

config_schema="$easybar_root/config.schema.json"
if [[ ! -f "$config_schema" ]]; then
  echo "EasyBar configuration schema not found: $config_schema" >&2
  exit 1
fi
